...
if [ "$chosen_network" = "" ]; then
    ...
    if [[ $(echo "$saved_connections" | grep -w "$chosen_id") = "$chosen_id" ]]; then
        nmcli connection up id "$chosen_id" | grep "successfully" && "$success_message"
    else
        if [[ "$chosen_network" =~ " " ]]; then
        ...
        nmcli device wifi connect "$chosen_id" password "$wifi_password" | grep "successfully" && "$success_message"
    fi
fi
